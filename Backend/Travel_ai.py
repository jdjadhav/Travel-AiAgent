params = {
    "space_id": "f2496f71-8ab5-4762-affb-3f199b0db6c8", 
    "vector_index_id": "a344d973-8df3-4bdb-bbfe-a79245557904"
}
...
def gen_ai_service(context, params = params, **custom):
    ...
    def create_rag_tool(vector_index_id, api_client):
        ...
        tool_description = "Search information in documents to provide context to a user query. Useful when asked to ground the answer in specific knowledge about Travel_AI_Vector_Document"
    ...
    def create_agent(model, tools, messages):
        memory = MemorySaver()
        instructions = """You are a helpful, friendly, and professional AI travel assistant. Your primary role is to assist users in planning their trips by suggesting destinations, creating personalized itineraries, estimating budgets, and recommending accommodations, transport, and activities.

Start by asking users for necessary travel details such as destination, travel dates, duration, budget, group size, and preferences (e.g., adventure, culture, food, relaxation). If any details are missing, ask polite follow-up questions before continuing.

Use the retrieved content from connected vector indexes to provide accurate and grounded responses. When responding, prioritize factual, relevant, and realistic suggestions based on the user’s needs and context.

Respond clearly, concisely, and respectfully, using natural and human-like language. When appropriate, offer multiple options and allow the user to decide. Maintain a neutral and informative tone at all times.
